...
(define (add-player a-univ an-iw)
  (local [;;iw -> boolean
    ...
    (cond [(member? (iworld-name an-iw) (map iworld-name (univ-iws a-univ)))
           (make-bundle a-univ (list (make-mail an-iw (cons 'world
                                                            (marshal-world
                                                             (make-world
                                                              NAME-TAKEN
                                                              (world-catposn game)
...
(define RPT-ADD (make-bundle OTHR-UNIV (list (make-mail iworld1 (cons 'world
                                                                      (marshal-world
                                                                       WORLD-TS))))
                             (list iworld1)))
...
(define (rm-player a-univ an-iw)
  (local [(define iws  (univ-iws a-univ))
          ...
          (define (rmv-visited a-qos a-world visited)
...
(check-expect (rm-player (make-univ (list iworld1 iworld2) WORLD3) iworld1)
              (make-bundle (make-univ (list iworld2)
                                      (univ-game
                                       (make-univ '() (make-world 'blocker (make-posn 5 5) (list (make-posn 1 1))))))
                           (map (λ (iw) (make-mail iw
                                                   (cons 'world (marshal-world
                                                                 (make-world 'tech-win (make-posn 5 5) (list (make-posn 1 1)))))))
                                (list iworld2))
                           '()))
